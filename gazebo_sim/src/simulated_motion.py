...
def init():
    global targeted_tf, tf_, shift_reg
    global p_v_pub, latest_common_time
    rospy.init_node('simulated_motion')
    targeted_tf = 'ref_frame'
    shift_reg = rospy.get_param('~shift_reg', 10)
...
def tf_callback(tf2):
    ...
    global shift_reg, sliding_window, sliding_window_v
    global p_v_pub, latest_common_time
    try:
        ...
        if latest_common_time < t:
            ...
            if len(sliding_window) >= shift_reg:
                del sliding_window[0]
                if len(sliding_window_v) >= shift_reg:
    ...
    if len(sliding_window_v) > shift_reg / 2 and len(sliding_window) > shift_reg / 2:
