...
def init():
    ...
    #subscribe to the msg generated by marker_vel_estimator.py              
    ...
    path_pub = rospy.Publisher('aerial_global_planner/plan', Path, queue_size=1)  #the UAV goal is published under this topic
...
def tf_callback(tf2):
    ...
    try:
        # in this segment, the transfomation between frames emulates the odometry pose estimation
        ...
        #Set the current pose of the UAV relative to the fixed /odom frame
...
def p_v_callback(pvmsg):
    ...
    if robot_pose != None:
        ...
        for v in pvmsg.latest_velocities:       #X and Y components of the velocity are taken from msg and stored in the empty arrays vx and vy
            vx.append(v.vx)
            vy.append(v.vy)
        #The mean of 10 latest velocities is computed, for each component
        ...
        while not stabilized:
            ...
            if len(vx) >= 3:                        # by default the condition is true since a msg containing 10 velocities was received, 
                                                    # after some iterations of the while loop the number of elements in vx and vy is reduced
                if abs(vx[0] - meanvx) >= 0.02:     # condition to check if the helipad is moving with constant velocity (elimination of noisy values) 
                    stabilized = False              # if the abs error between this vel and the media is grater than 0.02, delete vel
                    noise = True
                    del vx[0]
        ...
        #else:
        # The best velocity estimation is obtained
        lastvx = meanvx
        lastvy = meanvy
        #difference between the current time and the latest helipad's pose time
        ...
        if goalx != None:
            ...
            # publish the origin and goal of the path
            # origin: pose of the UAV
            # destiny: pose of the helipad (same orientation but with a mofified position, wich is equal to the best predicted future position)
            path_pub.publish(path_msg)              
            # convert the yaw to quaternion
            goal_quat = tf.transformations.quaternion_from_euler(0, 0, yaw)
            tf_broadcaster.sendTransform(       # braodcast the goal frame
                (goalx, goaly, goalz),
                goal_quat,
                rospy.Time.now(),
                'robot_goal',
...
def rendezvous(t_, helix, heliy, heliz, helivx, helivy, robotx, roboty, robotz, maxrobotv):
    ...
    goalz = None
    needed_velocities = []
    #loop to compute the helipad position in a range of future horizons, and the required velocities to reach them
    for t in float_range(t_,future_horizon, 0.1):      
        ...
        # if the needed velocities are lower than the max capable velociity of the UAV, store this vel values
    ...
    # loop through the needed velocities and determine the lowest xyz and xy velocities
    for i in needed_velocities:
        sumv = abs(i[0][0]) + abs(i[0][1]) + abs(i[0][2])       #Sum of the needed xyz velocities of one future horizon
        sumvxvy = abs(i[0][0]) + abs(i[0][1])       # Sum of the xy velocities of one future horizon
        # if the sum of velocities (magnitude of the velocity) is less than the max vel, set as goal
        # position the predicted helipad position in this future horizon
        if sumv < minv:                      
...
# get the difference between the latest helipad xy position and the goal computed before
